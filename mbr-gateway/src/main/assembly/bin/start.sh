...
LOGS_FILE=`sed '/boot.log.file/!d;s/.*=//' conf/application.properties | tr -d '\r'`
...
STDOUT_FILE=$LOGS_DIR/stdout-gateway.log
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS -jar $LIB_DIR/mbr-gateway-1.0-SNAPSHOT.jar --spring.config.location=$CONF_DIR/application.properties,$CONF_DIR/$ACTIVE_FILE, > $STDOUT_FILE 2>&1 &
